...
set -x
	...
	numberOfCores=12
        ...
        do
			...
			cp $rc slave.$machine
			coreCount=0	
			UDPPortNo=`expr $portNo + 10`
                        srun -p $partition --nodelist $machine -N1 -n8 --resv-ports=$portNo-$UDPPortNo --exclusive ./slave.$machine slave $ipAdd $portNo +RTS -l-au & 
                	sleep 2
	...
	coreCount=0	
	UDPPortNo=`expr $portNo + 10`
       	srun -p $partition --nodelist $getMasterIP -N1 -n8 --resv-ports=$portNo-$UDPPortNo --exclusive ./master.$getMasterIP master $ipAdd $portNo +RTS -l-au 
        #srun -p $partition --nodelist $getMasterIP -N1 -n1 --exclusive ./master.$getMasterIP master $ipAdd $portNo float_file.txt norm +RTS -lsgpfu 
