...
$PWD/dna_event.sh check $rc $JOBID $slurmJobNodeList
if [ $? -eq 0 ]; then
...
$PWD/dna_event.sh generatingCADFile $rc $JOBID $slurmJobNodeList 
if [ $? -eq 0 ]; then
...
$PWD/dna_event.sh md5sum $rc 0 0 
if [ $? -eq 0 ]; then
...
$PWD/dna_event.sh startingCHprocess $rc $JOBID machine.file.$JOBID 
if [ $? -eq 0 ]; then
...
$PWD/dna_event.sh collectLogs 0 $JOBID $slurmJobNodeList
if [ $? -eq 0 ]; then
